...
function pathToFile(path: string) {
  ...
  if (import.meta.env.DEV) {
    ...
    pagePath = inBrowser
      ? pagePath.slice(1).replace(/\//g, '_') + '.md'
      : 'app' + pagePath + '.md'
  }
  return pagePath
}
...
function createSearchTranslate(defaultTranslations: any) {
...
const enableFuzzySearch = useLocalStorage(
  'vitepress:local-search-fuzzy',
  false
)
...
debouncedWatch(
  () => [searchIndex.value, filterText.value, showDetailedList.value, enableFuzzySearch.value] as const,
...
.toggle-fuzzy-button.fuzzy-enabled {
